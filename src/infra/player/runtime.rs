...
use crate::infra::player::{PlayerEvent, StreamingPlayer};
...
#[cfg(all(feature = "mpris", target_os = "linux"))]
macro_rules! notify_mpris {
  ($mgr:expr, $method:ident, $($arg:expr),*) => {
    if let Some(ref mpris) = $mgr { mpris.$method($($arg),*); }
  };
}
...
#[cfg(all(feature = "macos-media", target_os = "macos"))]
macro_rules! notify_macos {
  ($mgr:expr, $method:ident, $($arg:expr),*) => {
    if let Some(ref macos) = $mgr { macos.$method($($arg),*); }
  };
}
#[cfg(not(all(feature = "macos-media", target_os = "macos")))]
macro_rules! notify_macos {
  ($mgr:expr, $method:ident, $($arg:expr),*) => {};
}

macro_rules! notify_both {
  ($mpris:expr, $macos:expr, $method:ident, $($arg:expr),*) => {
    notify_mpris!($mpris, $method, $($arg),*);
    notify_macos!($macos, $method, $($arg),*);
  };
}
    ...
    match event {
      PlayerEvent::Playing {
        ...
        if let Some(ref mut context) = app_lock.current_playback_context {
          ...
          context.progress = Some(TimeDelta::milliseconds(position_ms as i64));
        }

        app_lock.instant_since_last_current_playback_poll = std::time::Instant::now();

        let track_id_str = track_id.to_string();
        if app_lock.last_track_id.as_ref() != Some(&track_id_str) {
          app_lock.last_track_id = Some(track_id_str);
          app_lock.dispatch(IoEvent::GetCurrentPlayback);
        }
        if app_lock.pending_stop_after_track {
          app_lock.pending_stop_after_track = false;
          if let Some(ref mut context) = app_lock.current_playback_context {
      ...
      PlayerEvent::Seeked {
        ...
        notify_macos!(macos_media_manager, set_position, position_ms as u64);
        ...
        let (artists, album) = match &audio_item.unique_fields {
          UniqueFields::Track { artists, album, .. } => {
            let artist_names: Vec<String> =
              artists.0.iter().map(|artist| artist.name.clone()).collect();
      ...
      PlayerEvent::Stopped { .. } => {
        notify_both!(mpris_manager, macos_media_manager, set_stopped,);
      ...
      PlayerEvent::VolumeChanged { volume } => {
        ...
        if let Ok(mut app) = app.try_lock() {
          if let Some(pending) = app.pending_volume {
            ...
            if let Some(ref mut context) = app.current_playback_context {
              context.device.volume_percent = Some(volume_percent as u32);
            }
...
fn current_playback_matches_native(app: &App, player: &StreamingPlayer) -> bool {
  ...
  if let Some(native_id) = app.native_device_id.as_ref() {
    if context.device.id.as_ref() == Some(native_id) {
      return true;
    }
  }
  ...
  context
    .device
    .name
    .eq_ignore_ascii_case(player.device_name())
}
