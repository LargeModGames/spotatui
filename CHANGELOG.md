...
- **Experimental Sonos playback targets**: Sonos S1/S2 rooms discovered over local SSDP/UPnP now appear beside Spotify Connect devices in the Source & Device picker. Selected rooms persist across launches and receive local play/pause, previous/next, seek, volume, now-playing, and queue-add commands. Grouped-room transport is coordinator-aware; queue viewing, shuffle/repeat, and broader hardware testing remain follow-up work ([#251](https://github.com/LargeModGames/spotatui/issues/251)).
