...
- **One spotatui window at a time**: starting `spotatui` while another spotatui window runs now stops at once with "spotatui is already running (pid N)" instead of starting a second player beside the first. This includes `--reconfigure-auth` and `--play-file`: quit the running window first. CLI subcommands such as `spotatui playback` and `spotatui sync` still run beside it. Building from source now needs Rust 1.90 or newer.
...
- **Tokyo Night theme**: `preset: "Tokyo Night"` under `theme:` in `config.yml` (or the Theme row in Settings) switches every colour to the [Tokyo Night](https://github.com/folke/tokyonight.nvim) palette, next to the existing presets.
...
- **The Stats screen can be chosen as the startup screen from Settings**: `startup_route: "stats"` always worked when typed into `config.yml`, but the in-app Settings cycle never offered it. The cycle now matches the full startup-route list, and a test keeps the two from drifting again ([#443](https://github.com/LargeModGames/spotatui/issues/443)).
